...
@router.get("/sheets/{sheet_id}/lock", response_model=Optional[LockRead])
...
async def acquire_or_refresh_lock(
    ...
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User identification required")
