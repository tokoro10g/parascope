...
def generate_values(start: str | None, end: str | None, step: str | None, manual: List[str] | None) -> List[Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Start, End, and Increment values must be numeric strings."
        ) from err
...
async def sweep_sheet(
    ...
    if body.secondary_input_node_id:
        if str(body.secondary_input_node_id) not in node_map:
            raise HTTPException(
                status_code=400, detail=f"Secondary input node {body.secondary_input_node_id} not found."
            )
    ...
    if body.secondary_input_node_id:
        secondary_values = generate_values(
            body.secondary_start_value, body.secondary_end_value, body.secondary_increment, body.secondary_manual_values
        )
    ...
    if body.secondary_input_node_id:
        ...
        for sec_val in secondary_values:
            for prim_val in primary_values:
                scenarios.append({str(body.input_node_id): prim_val, str(body.secondary_input_node_id): sec_val})
    else:
        # 1D Sweep
        for prim_val in primary_values:
            scenarios.append({str(body.input_node_id): prim_val})

    if len(scenarios) > 2000:  # Allow slightly more for 2D
        raise HTTPException(
            status_code=400, detail=f"Sweep generates too many steps ({len(scenarios)}). Limit is 2000."
        )
    ...
    try:
        script = await generator.generate_sweep_script(
            root_sheet=sheet, scenarios=scenarios, static_overrides=static_overrides, output_node_ids=output_ids_str
        )
    ...
    return SweepResponse(
        headers=headers, results=results_rows, metadata=metadata_rows if metadata_rows else None, error=global_error
    )
