...
async def test_lock_lifecycle(client: AsyncClient):
    # 1. Create Sheet
    sheet_res = await client.post("/api/v1/sheets/", json={"name": "Lock Sheet"})
    sheet_id = sheet_res.json()["id"]

    # 2. Acquire Lock
    lock_data = {"tab_id": "tab1"}
    response = await client.post(
        f"/api/v1/sheets/{sheet_id}/lock",
        json=lock_data,
    ...
    # 3. Check status
    response = await client.get(f"/api/v1/sheets/{sheet_id}/lock")
    assert response.status_code == 200
    assert response.json()["user_id"] == "User1"
    ...
    # 7. Force Takeover
    response = await client.post(
        f"/api/v1/sheets/{sheet_id}/lock/force",
        json={"tab_id": "tab3"},
        headers={"X-Parascope-User": "User2"},
    )
    assert response.status_code == 200
    ...
    # 9. Release
    response = await client.delete(f"/api/v1/sheets/{sheet_id}/lock?tab_id=tab3", headers={"X-Parascope-User": "User2"})
    assert response.status_code == 200
