...
async def test_username_validation(client: AsyncClient):
    # Valid username
    response = await client.get("/api/v1/sheets/", headers={"X-Parascope-User": "valid_user_123"})
    ...
    response = await client.get("/api/v1/sheets/", headers={"X-Parascope-User": "invalid-user!"})
    ...
    response = await client.get("/api/v1/sheets/")
    assert response.status_code == 200
